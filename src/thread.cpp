...
void Thread::run(apr_pool_t* p, apr_thread_start_t start, void* data) {
    if (thread != NULL && !finished) {
		throw ThreadException(0);
	}
	apr_threadattr_t* attrs;
	apr_status_t stat = apr_threadattr_create(&attrs, p);
	if (stat != APR_SUCCESS) {
		throw ThreadException(stat);
	}
	stat = apr_thread_create(&thread, attrs, start, data, p);
	if (stat != APR_SUCCESS) {
		throw ThreadException(stat);
	}
}
